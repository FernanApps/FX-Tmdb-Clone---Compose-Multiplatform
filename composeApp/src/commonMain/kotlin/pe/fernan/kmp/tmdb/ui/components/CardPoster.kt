...
fun LazyItemScope.CardPoster(
    ...
    val cardHeight = 187.5.dp

    val spacer = 20.dp
    val heightTexts = 90.dp + spacer
    val circularProgressSize = 35.dp

    val height = cardHeight + circularProgressSize + heightTexts
    ...
    Box(modifier = Modifier.width(cardWidth).height(height)) {
        ...
        if (!loading) {
            ...
            Column(
                modifier = modifier.width(cardWidth).animateItemPlacement(
                ...
                Box {
                    Card(
                        modifier = Modifier.fillMaxWidth().height(cardHeight).clickable {
                            onClick(data)
                        }
                    ) {

                        val backDrop = Constant.IMAGE_BASE_PATH_CARD + data.backdropPath
                        val painter = rememberImagePainter(backDrop, placeholderPainter = {
                            placeholderPainter(
                                width = cardWidth.dpToPx(),
                    ...
                    Column(
                        modifier = Modifier.padding(
                            top = cardHeight - circularProgressSize / 2,
                            start = 7.5.dp,
                            end = 7.5.dp
                        )
